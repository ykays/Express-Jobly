...
describe("GET /companies", function () {
  ...
  test("with filter name", async function () {
  ...
  test("with filter name & minEmployees", async function () {
  ...
  test("with minEmployees only", async function () {
  ...
  test("with all filters", async function () {
    const resp = await request(app).get(
      "/companies?minEmployees=2&name=2&maxEmployees=3"
    );
    expect(resp.body).toEqual({
      companies: [
        {
  ...
  test("minEmployees is greater than maxEmployees-> error", async function () {
    const resp = await request(app).get(
      "/companies?minEmployees=4&name=2&maxEmployees=3"
    );
...
describe("GET /companies/:handle", function () {
  ...
  test("works for anon: company w/o jobs", async function () {
    const companyNoJobs = await Company.create({
      handle: "c8",
      name: "C8",
      description: "Desc8",
      numEmployees: 8,
...
describe("DELETE /companies/:handle", function () {
  ...
  test("unauthorized, not admin", async function () {
    const resp = await request(app)
      .delete(`/companies/c1`)
      .set("authorization", `Bearer ${u2Token}`);
